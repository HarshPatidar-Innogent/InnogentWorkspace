...
//	3	What is the average age of male and female employees?																								
	...
	public static void getAvgMaleFemale(List<Employee> employees) {
		...
		Double maleAge = employees.stream().filter(e -> e.getGender().equals("Male"))
				.collect(Collectors.averagingLong(Employee::getAge));
		Double femaleAge = employees.stream().filter(e -> e.getGender().equals("Female"))
				.collect(Collectors.averagingLong(Employee::getAge));
...
//	5	Get the names of all employees who have joined after 2015?																								
	public static List<Employee> getEmpOfYear(List<Employee> employees, int year) {
		List<Employee> employee = employees.stream().filter(e -> e.getYearOfJoining() == year)
...
//	8	Get the details of youngest male employee in the product development department?		
	public static Optional<Employee> getYoungestEmployee(List<Employee> employees) {
		return employees.stream().min(Comparator.comparingInt(Employee::getAge));
	}
...
//	13	Separate the employees who are younger or equal to 25 years from those employees who are older than 25 years?																								
	public static void getYoungEmployee(List<Employee> employees, int age) {
